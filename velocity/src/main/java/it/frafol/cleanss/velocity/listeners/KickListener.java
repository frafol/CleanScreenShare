...
public class KickListener {
    ...
    public void onPlayerConnect(ServerPreConnectEvent event) {
        ...
        if (PlayerCache.getSpectators().contains(player.getUniqueId())) {
            if (instance.useLimbo) return;
            if (Utils.isInControlServer(server)) return;
        ...
        if (PlayerCache.getSuspicious().contains(player.getUniqueId())
            ...
            if (Utils.isInControlServer(server) || instance.useLimbo) return;
    ...
    public void onPlayerConnected(PostLoginEvent event) {
        ...
        if (instance.getData() != null) instance.getData().setupPlayer(player.getUniqueId());
    ...
    public void onPlayerDisconnect(DisconnectEvent event) {
        ...
        if (!VelocityConfig.DISABLE_PING.get(Boolean.class)) servers = Utils.getOnlineServers(servers);
